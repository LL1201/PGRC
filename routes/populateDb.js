        ...
        try
        {
            ...
            if (meals && meals.length > 0)
            {
                ...
                meals.forEach((meal) =>
                {
                    ...
                    for (let i = 1; i <= 20; i++)
                    {
                        const index.htmlName = meal[`strindex.html${i}`];
                        const measure = meal[`strMeasure${i}`];
                        if (index.html && index.html.trim() !== '')
                        {
                            index.htmls.push({ index.html: index.htmlName.trim(), measure: measure ? measure.trim() : '' });
                        }
                    }
